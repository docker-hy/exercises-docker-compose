...
# success, the same way a student would.
set -uo pipefail
...
# Press the "redis" exercise button in the actual UI and check that it
# reports success, instead of calling /api/ping?redis=true directly.
...
node "$PLAYWRIGHT_DIR/check-exercise-button.mjs" "$APP_URL" redis \
  || fail "pressing the \"redis\" button in the frontend did not report success"
pass "pressing the \"redis\" button in the frontend reported success"
